...
def main() -> None:
    config_path = ROOT / "configs" / "baseline_sglang.yaml"
    config = yaml.safe_load(config_path.read_text(encoding="utf-8"))

    backend = SGLangBackend(
        model=str(_resolve_model_path(config["model"])),
        tp_size=int(config["engine"]["tensor_parallel_size"]),
        mem_fraction_static=float(config["engine"]["mem_fraction_static"]),
        context_length=int(config["engine"]["context_length"]),
    )
