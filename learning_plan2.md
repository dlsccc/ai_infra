...
- 精读论文，重点关注算法部分
- vllm/attention/backends/flash_attn.py源码
- 理解block_table数据结构

Day 3-4：Block Manager + Scheduler
- vllm/core/block_manager_v2.py
- vllm/core/scheduler.py
- 画数据结构图

Day 5-6：动手实验验证  
实验1：pagedattention vs native attention  
      测试不同batch size下显存占用  
      验证continuous batching优势  
实验2：修改vllm参数看性能变化（如修改block_size）  
必须搞懂：  
为什么不用连续内存？  
KV cache 如何分页？  
输出：  
...
**week8-9:Matmul优化（关键）**  
=== Week 8 ===  
Day 1-2: Naive + Shared Memory (1D)  
- Naive matmul实现  
- 1D tiling版本  
- 基础测试（正确性）  

Day 3-5: 2D Tiling ⭐ 关键  
- 实现2D blocking  
- 这里会有大量bug，预留足够时间  
- 测试不同tile size的性能  

Day 6-7: 性能分析 Round 1  
- nsys profile初步分析  
- 看SM occupancy为什么低  
- 找到第一批优化点  
...
=== Week 9 ===  
Day 1-3: 高级优化  
- Double buffering（如果Week 8有余力可提前）  
- Bank conflict优化  
- 目标: 达到10x naive性能  
